import argparse
import torch
from src.data_reader.dataset import RawDatasetGeoLife
from src.model.model import CDCK2ForGeoLife
...
def evaluate_model(args, logger):
    try:
        ...
        # Load the model
        model = CDCK2ForGeoLife(args.timestep, args.batch_size, args.trajectory_window,6).to(device)
        ...
        dataset = RawDatasetGeoLife(args.raw_hdf5, args.eval_list, args.trajectory_window)
...
def calculate_mean_rank(rank_matrix):
    ...
    for i in range(num_vectors):
        if i % 3 == 1 and i + 1 < num_vectors:
            ...
            if (rank_matrix[i, i + 1].item() == 1 or rank_matrix[i, i + 1].item() == 2 or rank_matrix[i, i - 1].item() == 2 or rank_matrix[i, i - 1].item() == 1):  
                ranks.append(1.0)
            else:
...
def main():
    ...
    parser.add_argument('--model-path', type=str, default='snapshot/geoLife/cdc-2024-09-09_23_24_04-model_best.pth', help='Path to the trained model file')
    parser.add_argument('--raw-hdf5', type=str, default='dataset/geoLife/processedGeolife.hdf5', help='Path to the HDF5 file containing raw data')
    parser.add_argument('--batch-size', type=int, default=8, help='Batch size for evaluation')
    parser.add_argument('--trajectory-window', type=int, default=512, help='Window length to sample from each utterance')
    ...
    parser.add_argument('--log-dir', type=str, default='evalSimilary/geoLifeSimilary/logs', help='Directory to save log files')
    parser.add_argument('--output-dir', type=str, default='evalSimilary/geoLifeSimilary/heatmaps', help='Directory to save heatmap images')
    args = parser.parse_args()
   
    logger = setup_logger(args.log_dir)
    
    files = [f for f in os.listdir('evalSimilary/geoLifeSimilary/subsetForVal') if f.endswith('.hdf5')]
    mean_ranks = []

    for file in files:
        hdf5_file = os.path.join('evalSimilary/geoLifeSimilary/subsetForVal', file)
