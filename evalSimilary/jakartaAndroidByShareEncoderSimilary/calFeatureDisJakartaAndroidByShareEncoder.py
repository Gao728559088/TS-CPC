...
import argparse
import torch
from src.data_reader.dataset import RawDatasetSingapore, RawDatasetSingaporeReverse
from src.model.model import CDCK6, CDCK2ForJakartaIos, CDCK2ForSingapore
...
def evaluate_model(args, logger):
    try:
        ...
        dataset = RawDatasetSingaporeReverse(args.raw_hdf5, args.eval_list, args.trajectory_window)
        data_loader = DataLoader(dataset, batch_size=args.batch_size, shuffle=False)

        features = []
        with torch.no_grad():
            for batch_data in tqdm(data_loader, desc="Extracting features", unit="batch"):
                batch_data = batch_data[0].float().to(device)
...
def calculate_mean_rank(rank_matrix):
    ...
    for i in range(num_vectors):
        if i % 3 == 1 and i + 1 < num_vectors:
            ...
            if (rank_matrix[i, i + 1].item() ==1 or rank_matrix[i, i + 1].item() == 0):  
                ranks.append(1.0)
            else:
...
def main():
    ...
    parser.add_argument('--model-path', type=str, default='snapshot/jakartaAndroidByShareEnconder/cdc-2024-09-25_23_20_44-model_best.pth', help='Path to the trained model file')
    parser.add_argument('--raw-hdf5', type=str, default='dataset/grab_possi/grab_possi_Jakarta_android/jakartaAndroid.hdf5', help='Path to the HDF5 file containing raw data')
    ...
    parser.add_argument('--log-dir', type=str, default='evalSimilary/jakartaAndroidByShareEncoderSimilary/logs', help='Directory to save log files')
    parser.add_argument('--output-dir', type=str, default='evalSimilary/jakartaAndroidByShareEncoderSimilary/heatmaps', help='Directory to save heatmap images')
    args = parser.parse_args()
    
    logger = setup_logger(args.log_dir)
    
    files = [f for f in os.listdir('evalSimilary/jakartaAndroidByShareEncoderSimilary/subsetForVal') if f.endswith('.hdf5')]
    mean_ranks = []

    for file in files:
        hdf5_file = os.path.join('evalSimilary/jakartaAndroidByShareEncoderSimilary/subsetForVal', file)
