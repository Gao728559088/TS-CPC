...
# 选择要使用的GPU
os.environ['CUDA_VISIBLE_DEVICES'] = '2,3'  # 设置使用GPU 0, 1, 2, 3
...
def process_plt_file(plt_file, trajectory_id, min_points=100, max_points=2000):
    try:
        # 读取文件并跳过前6行
        data = pd.read_csv(plt_file, skiprows=6, header=None,
                           names=['Latitude', 'Longitude', 'Zero', 'Altitude', 'Days', 'Date', 'Time'])

        data = data[['Latitude', 'Longitude', 'Altitude', 'Days']]
...
# 定义一个模型，用于处理数据，利用GPU加速
class GPUProcessor(nn.Module):
    def __init__(self):
        super(GPUProcessor, self).__init__()
...
# GPU处理函数
def gpu_process_data(data_chunk):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    gpu_processor = GPUProcessor()
    
    # 使用DataParallel来利用多个GPU
    if torch.cuda.device_count() > 1:
        gpu_processor = nn.DataParallel(gpu_processor)
    
    gpu_processor = gpu_processor.to(device)
    
    # 将数据转为tensor
    tensor_data = torch.tensor(data_chunk, dtype=torch.float32).to(device)
    processed_data = gpu_processor(tensor_data)
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for future in as_completed(futures):
            ...
            if result:
                ...
                data_to_write.append((dataset_name, gpu_process_data(data)))  # 利用GPU处理数据
            processed_files += 1
